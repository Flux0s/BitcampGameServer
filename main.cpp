...
bool isAClient(Client, Client[MAXCLIENTS]);
...
int main(void) {
    ...
    bool shutdownServer = false, addingClients = true;
    ...
    while (!shutdownServer) {
        ...
        if (std::string(buf).substr(0, 2) == NEWCLIENT && !isAClient(newClient, clients) && numClients < MAXCLIENTS) {
